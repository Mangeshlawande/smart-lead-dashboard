 ...
 * ------------------------------------------------------------------
 * This project intentionally runs with NODE_ENV=development on Render.
 *
 * Reason:
 * TypeScript compilation and runtime tooling depend on devDependencies
 * such as:
 * - typescript
 * - @types/*
 * - ts-jest
 * - jest
 *
 * Render skips devDependencies when NODE_ENV=production, which causes:
 * - missing TypeScript types
 * - failed builds
 * - missing Jest/node typings
 *
 * For this assignment/demo deployment, keeping NODE_ENV=development
 * ensures stable TypeScript builds on Render.
 *
 * In a real production setup:
 * - build should happen in CI
 * - compiled JS only should be deployed
 * - devDependencies should not be required at runtime
 * ------------------------------------------------------------------
 */
...
const envSchema = z.object({
  ...
  // Optional preview/staging origins
